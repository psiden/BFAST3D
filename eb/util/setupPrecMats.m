...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
function [QList,GList] = setupPrecMats(QTypes,N,sz,bmask,ndim)
...
%             Ry = spdiags([-1*ones(sz(2),1),2*ones(sz(2),1),-1*ones(sz(2),1)],[-1,0,1],sz(2),sz(2));
%             Rz = spdiags([-1*ones(sz(3),1),2*ones(sz(3),1),-1*ones(sz(3),1)],[-1,0,1],sz(3),sz(3));
%             Lxy = kron(Ry,speye(sz(1))) + kron(speye(sz(2)),Rx);
%             Limage = kron(Rz,speye(sz(1)*sz(2))) + kron(speye(sz(3)),Lxy);
%             L = Limage(bmask,bmask);
%             L = L - diag(sum(L));
%             QList{k} = L;
%             GList{k} = '';
