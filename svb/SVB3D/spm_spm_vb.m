...
%     block_template(s).verbose       = 0;
    ...
    block_template(s).SVB.ParallelGMRFSampling = SPM.ParallelGMRFSampling;
end
...
for z = 1:nLb
    ...
    for s = 1:nsess
        ...
        % Added by Per Siden 2016-06-09 (Include mask info to block)
        block.SVB.sz = SPM.xY.VY(1).dim;
        if vol;       
            block.SVB.ndim = 3;
            block.SVB.bmask = find(mask);
        else       
            block.SVB.ndim = 2;
            block.SVB.sz = block.SVB.sz(1:2);
            sliceNbr = zz(1);
            mask2d = squeeze(mask(:,:,sliceNbr));
            block.SVB.bmask = find(mask2d);
        end
        ...
        % Added by Per Siden 160609
        SPM.SVB(z).Ns = block.SVB.Ns;
        SPM.SVB(z).NsWarmup = block.SVB.NsWarmup;
        SPM.SVB(z).PCGwIterSave = block.SVB.PCGwIterSave;
        SPM.SVB(z).PCGaIterSave = block.SVB.PCGaIterSave;
        SPM.SVB(z).warmupIterations = block.SVB.warmupIterations;
        SPM.SVB(z).BTilde = block.SVB.BTilde;
        SPM.SVB(z).BI = block.SVB.BI;
        SPM.SVB(z).JTilde = block.SVB.JTilde;
        SPM.SVB(z).JI = block.SVB.JI;
        SPM.SVB(z).wSampSim = block.SVB.wSampSim;
        SPM.SVB(z).Y = R0Y; % HP-filtered Y
        SPM.SVB(z).R0 = hpf(s).R0; % HP-filtering matrix
        SPM.SVB(z).SPMalpha = block.mean_alpha; % VB estimated alpha
        SPM.SVB(z).SPMb_alpha = block.b_alpha;  % VB alpha gamma scale
        SPM.SVB(z).SPMc_alpha = block.c_alpha;  % VB alpha gamma shape
        SPM.SVB(z).SPMbeta = block.mean_beta; % VB estimated beta
        SPM.SVB(z).SPMb_beta = block.b_beta;  % VB beta gamma scale
        SPM.SVB(z).SPMc_beta = block.c_beta;  % VB beta gamma shape
        SPM.SVB(z).alphaSave = block.SVB.alphaSave;
        SPM.SVB(z).betaSave = block.SVB.betaSave;
        SPM.SVB(z).w_mean = block.w_mean;
        SPM.SVB(z).timeTaken = timeTaken;
        SPM.SVB(z).timeTakenVec = block.SVB.timeTakenVec;
        clear block;
